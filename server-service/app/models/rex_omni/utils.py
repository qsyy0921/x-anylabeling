...
class ColorGenerator:
    ...
    def __init__(self, color_type: str = "text"):
        ...
        if color_type == "same":
            self.color = tuple(
                (np.random.randint(0, 127, size=3) + 128).tolist()
            )
        elif color_type == "text":
            np.random.seed(3396)
            self.num_colors = 300
            self.colors = (
                np.random.randint(0, 127, size=(self.num_colors, 3)) + 128
            )
        else:
    ...
    for category, annotations in predictions.items():
        ...
        for i, annotation in enumerate(annotations):
            ...
            if annotation_type == "box" and len(coords) == 4:
                ...
                _draw_keypoint(
                    draw, annotation, color, draw_width, font, show_labels
                )
...
def _draw_polygon(
    ...
    if show_labels and label and coords:
        ...
        # Draw text
        draw.text(
            (label_x, label_y - box_h - 2), label, fill="black", font=font
        )
...
def _draw_keypoint(
    ...
    if len(bbox) == 4:
        _draw_box(
            draw, bbox, color, draw_width, instance_id, font, show_labels
        )
    ...
    for category, annotations in predictions.items():
        ...
        for i, annotation in enumerate(annotations):
            ...
            if ann_type == "box" and len(coords) == 4:
                x0, y0, x1, y1 = coords
                lines.append(
                    f"  Box {i+1}: ({x0:.1f}, {y0:.1f}, {x1:.1f}, {y1:.1f})"
                )
                ...
                keypoints = annotation.get("keypoints", {})
                visible_kps = sum(
                    1 for kp in keypoints.values() if kp != "unvisible"
                )
                lines.append(
