...
class Evaluator:
    ...
    def __init__(self, config=None):
        ...
        self.config = utils.init_config(
            config, self.get_default_eval_config(), "Eval"
        )
    ...
    def _combine_results(
        ...
        if target_tag is not None:
            ...
            target_video = [
                vid2name[video_id] for video_id in target_video_ids
            ]
            ...
            assert all(
                target_tag in annot["tags"] for annot in target_annotations
            ), (
        ...
        for c_cls in class_list:
            ...
            for metric, metric_name in zip(metrics_list, metric_names):
                ...
                res[res_field][c_cls][metric_name] = metric.combine_sequences(
                    curr_res
                )
        # combine classes
            ...
            if config["OUTPUT_EMPTY_CLASSES"] or num_dets > 0:
                for metric, metric_name in zip(metrics_list, metric_names):
                    ...
                    if c_cls in combined_cls_keys:
                        table_res = {
                            res_field: res[res_field][c_cls][metric_name]
                        }
                    else:
                        table_res = {
                            seq_key: seq_value[c_cls][metric_name]
