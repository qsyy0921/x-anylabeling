...
def update_config(config):
    ...
    for setting in config.keys():
        if type(config[setting]) == list or type(config[setting]) == type(
            None
        ):
...
def write_detailed_results(details, cls, output_folder):
    ...
    fields = ["seq"] + sum(
        [list(s["COMBINED_SEQ"].keys()) for s in details], []
    )
    ...
    with open(out_file, "w", newline="") as f:
        ...
        for seq in sorted(sequences):
            if seq == "COMBINED_SEQ":
                continue
            writer.writerow(
                [seq] + sum([list(s[seq].values()) for s in details], [])
            )
        writer.writerow(
            ["COMBINED"]
            + sum([list(s["COMBINED_SEQ"].values()) for s in details], [])
        )
