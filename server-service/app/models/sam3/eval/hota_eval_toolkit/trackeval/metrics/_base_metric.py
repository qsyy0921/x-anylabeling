...
class _BaseMetric(ABC):
    ...
    def combine_classes_class_averaged(
        self, all_res, ignore_empty_classes=False
    ): ...
    ...
    def plot_single_tracker_results(
        self, all_res, tracker, output_folder, cls
    ):
        """Plot results of metrics, only valid for metrics with self.plottable"""
        if self.plottable:
            raise NotImplementedError(
                "plot_results is not implemented for metric %s"
                % self.get_name()
            )
        else:
            pass
    ...
    def _combine_weighted_av(all_res, field, comb_res, weight_field):
        ...
        return sum(
            [
                all_res[k][field] * all_res[k][weight_field]
    ...
    def summary_results(self, table_res):
        ...
        return dict(
            zip(
                self.summary_fields,
                self._summary_row(table_res["COMBINED_SEQ"]),
            )
        )
    ...
    def _detailed_row(self, res):
        ...
        for h in self.float_array_fields + self.integer_array_fields:
            for i, alpha in enumerate(
                [int(100 * x) for x in self.array_labels]
            ):
