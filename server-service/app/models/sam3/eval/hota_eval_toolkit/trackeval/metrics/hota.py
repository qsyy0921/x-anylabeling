...
class HOTA(_BaseMetric):
        ...
        for t, (gt_ids_t, tracker_ids_t) in enumerate(
            ...
            for a, alpha in enumerate(self.array_labels):
                ...
                if num_matches > 0:
                    ...
                    matches_counts[a][
                        gt_ids_t[alpha_match_rows],
                        tracker_ids_t[alpha_match_cols],
                    ] += 1
    ...
    def combine_sequences(self, all_res):
        ...
        loca_weighted_sum = sum(
            [
                all_res[k]["LocA"] * all_res[k]["HOTA_TP"]
    ...
    def combine_classes_class_averaged(
        ...
        for field in self.float_fields + self.float_array_fields:
            if ignore_empty_classes:
                ...
                res[field] = np.mean(
                    [v[field] for v in all_res.values()], axis=0
                )
        return res
    ...
    def plot_single_tracker_results(
        self, table_res, tracker, cls, output_folder
    ):
        ...
        for name in self.float_array_fields:
            legend += [
                name + " (" + str(np.round(np.mean(res[name]), 2)) + ")"
            ]
