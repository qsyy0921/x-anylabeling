...
class YTVIS(COCO):
    ...
    @override
    def __init__(
        self, annotation_file: str = None, ignore_gt_cats: bool = True
    ):
        """
        Args:
    ...
    def loadRes(self, resFile):
        ...
        if "bboxes" in anns[0] and not anns[0]["bboxes"] == []:
            ...
            for id, ann in enumerate(anns):
                bbs = [
                    (bb if bb is not None else [0, 0, 0, 0])
                    for bb in ann["bboxes"]
                ]
                xxyy = [
                    [bb[0], bb[0] + bb[2], bb[1], bb[1] + bb[3]] for bb in bbs
                ]
                if not "segmentations" in ann:
                    ann["segmentations"] = [
                        [[x1, y1, x1, y2, x2, y2, x2, y1]]
                        for (x1, x2, y1, y2) in xxyy
                    ]
    ...
    def annToRLE(self, ann):
        raise NotImplementedError(
            "We expect masks to be already in RLE format"
        )
