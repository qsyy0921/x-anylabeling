...
def _get_metric_index(
    metric_name: str, iou_threshold: Optional[float] = None
) -> int:
    """
    ...
    for idx, metric in enumerate(CGF1_METRICS):
        if (
            ...
            and metric.iou_threshold == iou_threshold
        ):
            return idx
    raise ValueError(
...
class YTVISPredFileEvaluator(BasePredFileEvaluator):
    ...
    def __init__(
        ...
        self.iou_types = (
            list(iou_types) if iou_types is not None else ["bbox", "segm"]
        )
...
class VideoPhraseApEvaluator(BasePredFileEvaluator):
    ...
    def evaluate(self, pred_file: str) -> Dict[str, float]:
        ...
        for iou_type in self.iou_types:
            ...
            phraseApEval.params.areaRngLbl = [
                "all",
                "small",
                "medium",
                "large",
            ...
            result_prefix += (
                f"_{'mask' if iou_type == 'segm' else 'bbox'}_phrase_ap"
            )
            ...
            results[result_prefix + "_50_95"] = phraseApEval.stats[
                0
            ]  # IoU=0.5:0.95
...
class VideoCGF1Evaluator(BasePredFileEvaluator):
    ...
    def evaluate(self, pred_file: str) -> Dict[str, float]:
        ...
        video_np_level_results = {}
        for iou_type in self.iou_types:
            demoF1Eval = VideoDemoF1Eval(
                ytvisGT, ytvisDT, iou_type, self.prob_thresh
            )
            ...
            result_prefix = f"{self.dataset_name}"
            result_prefix += (
                f"_{'mask' if iou_type == 'segm' else 'bbox'}_demo"
            )
            ...
            if compute_ilmcc_and_cgf1:
                ...
                positive_micro_f1_avg_idx = _get_metric_index(
                    "positive_micro_F1", None
                )
                ilmcc_avg_idx = _get_metric_index("IL_MCC", None)
                results[result_prefix + "_cgf1_micro_50_95"] = stats[
                    cgf1_micro_avg_idx
                ]
                ...
                positive_micro_f1_50_idx = _get_metric_index(
                    "positive_micro_F1", 0.5
                )
                results[result_prefix + "_cgf1_micro_50"] = stats[
                    cgf1_micro_50_idx
                ]
                ...
                positive_micro_f1_75_idx = _get_metric_index(
                    "positive_micro_F1", 0.75
                )
                results[result_prefix + "_cgf1_micro_75"] = stats[
                    cgf1_micro_75_idx
                ]
            ...
            self.extract_video_np_level_results(
                demoF1Eval, video_np_level_results
            )

        return results, video_np_level_results

    def extract_video_np_level_results(
        self, demoF1Eval, video_np_level_results
    ):
        ...
        result_prefix = (
            "mask" if demoF1Eval.params.iouType == "segm" else "bbox"
        )

        assert len(demoF1Eval.evalImgs) == len(
            demoF1Eval.cocoGt.dataset["images"]
        )
        for i, video in enumerate(demoF1Eval.cocoGt.dataset["images"]):
            ...
            TPs = eval_img_dict.get(
                "TPs", np.zeros(num_iou_thrs, dtype=np.int64)
            )
            FPs = eval_img_dict.get(
                "FPs", np.zeros(num_iou_thrs, dtype=np.int64)
            )
            FNs = eval_img_dict.get(
                "FNs", np.zeros(num_iou_thrs, dtype=np.int64)
            )
            ...
            F1s = np.where(
                denominator > 0, 2 * TPs / np.maximum(denominator, 1), 1.0
            )
            local_results = {
            ...
            video_np_level_results[(video_id, category_id)].update(
                local_results
            )
...
class VideoTetaEvaluator(BasePredFileEvaluator):
    ...
    def __init__(
        ...
        print(
            f"Initialized VideoTetaEvaluator with NMS strategy: {self.nms_strategy}"
        )
    ...
    def process_predictions(self, pred_file: str, tmp_dir: str) -> str:
        ...
        if self.prob_thresh > 0:
            raw_preds = [
                d for d in raw_preds if d["score"] >= self.prob_thresh
            ]
            print(
        ...
        if self.nms_strategy == "track":
            process_track_level_nms(
                video_groups, nms_threshold=self.nms_threshold
            )
    ...
    def evaluate(self, pred_file: str) -> Tuple[Dict[str, float], Dict]:
        ...
        print(
            f"Evaluating TETA Metric with {self.nms_strategy.upper()} NMS strategy"
        )
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            pred_coco_path = os.path.join(
                tracker_dir, "track_results_cocofmt.json"
            )
            convert_ytbvis_to_cocovid_pred(
...
class VideoPhraseHotaEvaluator(BasePredFileEvaluator):
        ...
        for d in dt:
            ...
            for t in range(len(d["bboxes"])):
                ...
                if (
                    d["areas"][t] == 0
                    or bbox is None
                    or all(x == 0 for x in bbox)
                ):
                    d["segmentations"][t] = None
                    d["bboxes"][t] = None
                    d["areas"][t] = None
            # check that box occurence and mask occurence are consistent
            for bbox, mask, area in zip(
                d["bboxes"], d["segmentations"], d["areas"]
            ):
        ...
        for iou_type in self.iou_types:
            ...
            def _summarize_results(output_res, iou_type, field, suffix):
                ...
                for metric_name in self.metric_to_collect:
                    ...
                    result_key = (
                        f"{result_prefix}_{self.metric_prefix}_{metric_name}"
                    )
            ...
            if (
                ...
                in output_res[self.dataset_name]["tracker"]
            ):
                _summarize_results(
                    output_res,
                    iou_type,
    ...
    def extract_video_np_level_results(
        ...
        for video in remapped_gt["videos"]:
            ...
            results = raw_results[video_key]["_REMAPPED_FOR_PHRASE_METRICS_"][
                "HOTA"
            ]
...
def remap_gt_dt_class_agnostic(gt, dt):
    ...
    for video in gt["videos"]:
        ...
        video["file_names"] = [
            f"remapped_vid_{video['id']:012d}/{name}"
            for name in video["file_names"]
        ]
