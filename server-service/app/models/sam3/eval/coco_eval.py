...
class CocoEvaluator:
    def __init__(
        ...
        assert (
            len(maxdets) == 3
        ), f"expecting 3 detection threshold, got {len(maxdets)}"
    ...
    def _lazy_init(self, coco_cls=COCO):
        ...
        self.coco_gts = [
            (
                coco_cls(g_pathmgr.get_local_path(gt))
        ...
        if self.average_by_rarity:
            ...
            eval_img_ids_set = (
                set(self.eval_img_ids)
                if self.eval_img_ids is not None
                else None
            )
            for img in self.coco_gts[0].dataset["images"]:
                if (
                    self.eval_img_ids is not None
                    and img["id"] not in eval_img_ids_set
                ):
                    continue
    ...
    def update(self, *args, **kwargs):
        ...
        for iou_type in self.iou_types:
            ...
            all_scorings = []
            for cur_coco_gt, cur_coco_eval in zip(
                self.coco_gts, self.coco_evals
            ):
                # suppress pycocotools prints
                with open(os.devnull, "w") as devnull:
                    with contextlib.redirect_stdout(devnull):
                        coco_dt = (
                            self._loadRes(cur_coco_gt, results)
                            if results
                            else COCO()
                        )
                ...
                img_ids, eval_imgs = self._evaluate(
                    coco_eval, self.use_self_evaluate
                )
    ...
    def synchronize_between_processes(self):
        ...
        if self.dump is not None:
            dumped_file = (
                Path(self.dump_dir) / f"coco_predictions_{get_rank()}.json"
            )
            logging.info(
                f"COCO evaluator: Dumping local predictions to {dumped_file}"
            )
    ...
    def accumulate(self, imgIds=None):
        ...
        if imgIds is not None:
            ...
            for coco_eval in self.coco_evals[0].values():
                p = coco_eval.params
                id_mask = np.array(
                    [(i in imgIds) for i in p.imgIds], dtype=bool
                )
    ...
    def summarize(self):
        ...
        if self.rarity_buckets is None:
            ...
            for bucket, img_list in self.rarity_buckets.items():
                ...
                for iou_type, coco_eval in self.coco_evals[0].items():
                    print(
                        f"IoU metric: {iou_type}. Rarity bucket: {bucket_name}"
                    )
                    summarize(coco_eval)
    ...
    def reset(self, cocoeval_cls=COCOeval):
        ...
        for i, coco_gt in enumerate(self.coco_gts):
            for iou_type in self.iou_types:
                self.coco_evals[i][iou_type] = cocoeval_cls(
                    coco_gt, iouType=iou_type
                )
    ...
    def write(self, stats):
        ...
        if "segm" in self.coco_evals[0]:
            stats["coco_eval_masks"] = self.coco_evals[0][
                "segm"
            ].stats.tolist()
        return stats
...
def summarize(self):
    ...
    def _summarizeDets():
        ...
        for area in self.params.areaRngLbl[1:]:
            stats[cur_id] = _summarize(
                1, areaRng=area, maxDets=self.params.maxDets[2]
            )
            cur_id += 1
        ...
        for area in self.params.areaRngLbl[1:]:
            stats[cur_id] = _summarize(
                0, areaRng=area, maxDets=self.params.maxDets[2]
            )
            cur_id += 1
