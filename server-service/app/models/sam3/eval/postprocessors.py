...
class PostProcessImage(nn.Module):
    ...
    def _process_masks(
        self, target_sizes, pred_masks, consistent=True, keep=None
    ):
        ...
        if consistent:
            ...
            ), "Expecting all target sizes to be equal"
            out_masks = (
                interpolate(
                    pred_masks,
    ...
    def process_results(
        ...
        **kwargs,
        ...
        if find_stages.loss_stages is not None:
            find_metadatas = [
                find_metadatas[i] for i in find_stages.loss_stages
            ]
        ...
        for outputs, meta in zip(find_stages, find_metadatas):
            ...
            for img_id, result in zip(ids, detection_results):
                if img_id.item() not in results:
                    ...
                    assert set(results[img_id.item()].keys()) == set(
                        result.keys()
                    )
            ...
            if (
                ...
                for k in result.keys():
                    if isinstance(results[img_id][k], list):
                        results[img_id][k] = [
                            results[img_id][k][i]
                            for i in topk_indexes.tolist()
                        ]
                    else:
                        results[img_id][k] = results[img_id][k].to(
                            topk_indexes.device
                        )[topk_indexes]
    ...
    def process_results(
        ...
        **kwargs,
        ...
        for frame_idx, (frame_outs, meta) in enumerate(
            ...
            assert obj_ids_per_frame.size(-1) == frame_outs[
                "pred_logits"
            ].size(-2)
            ...
            if self.always_interpolate_masks_on_gpu:
                ...
                assert gpu_device.type == "cuda"
                tracked_objs_outs_td = tracked_objs_outs_td.to(
                    device=gpu_device
                )
            ...
            if self.convert_mask_to_rle_for_video:
                ...
                rle_list = rle_encode(
                    interpolated_binary_masks, return_areas=True
                )
        ...
        padded_frames_results["scores"][
            ...
        ] = -1e8  # a very low score for empty object
        ...
        if self.convert_mask_to_rle_for_video:
            vid_masklets_rle_padded = [
                [None] * num_frames for _ in range(num_preds)
            ]
        for o_idx, oid in enumerate(tracked_objects_packed_idx):
            ...
            if self.convert_mask_to_rle_for_video:
                for packed_idx, padded_idx in zip(
                    oid2packed_idx, oid2padded_idx
                ):
...
class PostProcessTracking(PostProcessImage):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            max_dets_per_img=max_dets_per_img, iou_type=iou_type, **kwargs
        )
    ...
    def process_results(
        ...
        for outputs, meta in zip(find_stages, find_metadatas):
            ...
            detection_results = self(
                outputs, meta.original_size, consistent=False
            )
