...
class VEvalEvaluator:
    def __init__(self, gt_annot_file: str, eval_res_file: str):
        ...
        self.evaluators = [
            ...
            # TETA
            VideoTetaEvaluator(
                gt_annot_file, use_mask=True, is_exhaustive=True
            ),
            # HOTA
...
def run_main_all(dataset_name, args):
    ...
    eval_res_file = os.path.join(
        args.eval_res_dir, dataset_name + "_eval_res.json"
    )
    print(
        f"=== Running evaluation for Pred {pred_file} vs GT {gt_annot_file} ==="
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run video grounding evaluators"
    )
    ...
    all_parser = subparsers.add_parser(
        "all", help="Run evaluation for all datasets"
    )
    ...
    one_parser = subparsers.add_parser(
        "one", help="Run evaluation for one dataset"
    )
