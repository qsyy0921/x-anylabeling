...
CGF1_METRICS = [
    ...
    Metric(
        name="positive_micro_precision", image_level=False, iou_threshold=None
    ),
    ...
    Metric(
        name="positive_micro_precision", image_level=False, iou_threshold=0.5
    ),
    ...
    Metric(
        name="positive_micro_precision", image_level=False, iou_threshold=0.75
    ),
]
...
class CGF1Eval(COCOeval):
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        assert (
            not p.useCats
        ), "This evaluator does not support per-category evaluation."
        assert catId == -1
        ...
        keep_dt = np.array(
            [d["score"] >= self.threshold for d in all_dts], dtype=bool
        )
        ...
        if len(gt) > 0 and len(dt) == 0:
            ...
            return {
                ...
                "local_positive_F1s": np.zeros(
                    (len(p.iouThrs),), dtype=np.int64
                ),
                "num_dt": len(dt),
            }
    ...
    def accumulate(self, p=None):
        ...
        self.eval["cgF1"] = (
            self.eval["positive_micro_F1"] * self.eval["IL_MCC"]
        )
        ...
        def _summarizeDets():
            ...
            for metric in CGF1_METRICS:
                if metric.image_level:
                    ...
                    stats.append(
                        _summarize(
                            iouThr=metric.iou_threshold, metric=metric.name
                        )
                    )
...
class CGF1Evaluator:
    ...
    def __init__(
        ...
        self.eval_img_ids = [
            ...
            if (
                img["is_instance_exhaustive"]
                and img["id"] not in exclude_img_ids
            )
        ]
    ...
    def evaluate(self, pred_file: str):
        ...
        assert (
            len(self.coco_gts) > 0
        ), "No ground truth provided for evaluation."
        ...
        self.coco_evals[0]._paramsEval = copy.deepcopy(
            self.coco_evals[0].params
        )
