...
def uniform_positive_sample(mask, n_points):
    ...
    assert (
        len(mask_points) > 0
    ), "Can't sample positive points from an empty mask."
    selected_idxs = np.random.randint(
        low=0, high=len(mask_points), size=n_points
    )
            ...
            if n_points > 0:
                ...
                if self.concat_points and query.input_points is not None:
                    input_points = torch.cat(
                        [query.input_points, input_points], dim=1
                    )
            else:
                input_points = (
                    query.input_points if self.concat_points else None
                )
