...
def crop(
    ...
    if v2:
        ...
        datapoint.images[index].data = F.crop(
            datapoint.images[index].data, *region
        )
    ...
    for obj in datapoint.images[index].objects:
        ...
        if recompute_box_from_mask and obj.segment is not None:
            ...
            obj.bbox, obj.area = get_bbox_xyxy_abs_coords_from_mask(
                obj.segment
            )
        else:
            if (
                recompute_box_from_mask
                and obj.segment is None
                and obj.area > 0
            ):
                logging.warning(
            ...
            cropped_boxes = boxes - torch.as_tensor(
            ...
            obj.area = (cropped_boxes[:, 1, :] - cropped_boxes[:, 0, :]).prod(
                dim=1
            )
    ...
    for query in datapoint.find_queries:
        ...
        if query.image_id == index and query.input_points is not None:
            ...
            cropped_points = points - torch.as_tensor(
                [j, i, 0], dtype=torch.float32
            )
            cropped_points[:, :, :2] = torch.min(
                cropped_points[:, :, :2], max_size
            )
...
def hflip(datapoint, index):
    ...
    for query in datapoint.find_queries:
        ...
        if query.image_id == index and query.input_points is not None:
            points = query.input_points
            points = points * torch.as_tensor([-1, 1, 1]) + torch.as_tensor(
                [w, 0, 0]
            )
...
def resize(datapoint, index, size, max_size=None, square=False, v2=False):
    ...
    if v2:
        ...
        datapoint.images[index].data = F.resize(
            datapoint.images[index].data, size
        )
    ...
    ratios = tuple(
        float(s) / float(s_orig) for s, s_orig in zip(new_size, old_size)
    )
        ...
        if obj.segment is not None:
            if v2:
                if len(padding) == 2:
                    ...
                    obj.segment = Fv2.pad(
                        obj.segment[None], tuple(padding)
                    ).squeeze(0)
            else:
                if len(padding) == 2:
                    obj.segment = F.pad(
...
class RandomSizeCropAPI:
    ...
    def _sample_respect_boxes(self, img, boxes, points, min_box_size=10.0):
        ...
        maxX = (
            torch.cat([boxes[:, 2] - min_box_size, points[:, 0]], dim=0)
            .min()
            .item()
        )
        maxY = (
            torch.cat([boxes[:, 3] - min_box_size, points[:, 1]], dim=0)
            .min()
            .item()
        )
        maxX = max(0.0, maxX)
    ...
    def __call__(self, datapoint, **kwargs):
        if self.respect_boxes or self.respect_input_boxes:
            if self.consistent_transform:
                ...
                for i in range(len(datapoint.images)):
                    ...
                    if self.respect_boxes:
                        all_boxes += [
                            obj.bbox.view(-1, 4)
                            for obj in datapoint.images[i].objects
                        ]
            ...
            if self.consistent_transform:
                ...
                crop_param = self._sample_no_respect_boxes(
                    datapoint.images[0].data
                )
                for i in range(len(datapoint.images)):
                    datapoint = crop(
                        datapoint,
...
class CenterCropAPI:
    def __init__(
        self, size, consistent_transform, recompute_box_from_mask=False
    ):
        self.size = size
        self.consistent_transform = consistent_transform
        self.recompute_box_from_mask = recompute_box_from_mask
    ...
    def __call__(self, datapoint, **kwargs):
        if self.consistent_transform:
            ...
            crop_top, crop_left, crop_height, crop_width = self._sample_crop(
                w, h
            )
            for i in range(len(datapoint.images)):
                datapoint = crop(
...
class ScheduledRandomResizeAPI(RandomResizeAPI):
    def __init__(self, size_scheduler, consistent_transform, square=False):
        ...
        super().__init__(
            sizes, consistent_transform, max_size=max_size, square=square
        )

    def __call__(self, datapoint, **kwargs):
        assert (
            "epoch" in kwargs
        ), "Param scheduler needs to know the current epoch"
        params = self.size_scheduler(kwargs["epoch"])
        sizes, max_size = params["sizes"], params["max_size"]
        self.sizes = sizes
        self.max_size = max_size
        datapoint = super(ScheduledRandomResizeAPI, self).__call__(
            datapoint, **kwargs
        )
        return datapoint
...
class PadToSizeAPI:
    def __init__(
        self, size, consistent_transform, bottom_right=False, v2=False
    ):
        self.size = size
        self.consistent_transform = consistent_transform
        self.v2 = v2
...
class RandomMosaicVideoAPI:
    ...
    def __call__(self, datapoint, **kwargs):
        ...
        if self.use_random_hflip:
            ...
            should_hflip = torch.zeros(
                self.grid_h, self.grid_w, dtype=torch.bool
            )
        for i in range(len(datapoint.images)):
...
def random_mosaic_frame(
    ...
    for grid_y in range(grid_h):
        for grid_x in range(grid_w):
            ...
            if (H_im_downsize, W_im_downsize) in downsize_cache:
                image_data_downsize = downsize_cache[
                    (H_im_downsize, W_im_downsize)
                ]
            else:
                ...
                downsize_cache[(H_im_downsize, W_im_downsize)] = (
                    image_data_downsize
                )
            ...
            if is_pil:
                image_data_output.paste(
                    image_data_downsize, (x_offset_b, y_offset_b)
                )
            else:
                image_data_output[
                    :, y_offset_b:y_offset_e, x_offset_b:x_offset_e
                ] = image_data_downsize
    ...
    for obj in datapoint.images[index].objects:
        if obj.segment is None:
            continue
        assert (
            obj.segment.shape == (H_im, W_im)
            and obj.segment.dtype == torch.uint8
        )
    ...
    def __call__(self, datapoint: Datapoint, **kwargs):
        for img in datapoint.images:
            if self.v2:
                ...
                img.data = Fv2.normalize(
                    img.data, mean=self.mean, std=self.std
                )
            else:
        ...
        for query in datapoint.find_queries:
            ...
            if query.input_points is not None:
                ...
                points = points / torch.tensor(
                    [cur_w, cur_h, 1.0], dtype=torch.float32
                )
                query.input_points = points
...
class ColorJitter:
    def __init__(
        self, consistent_transform, brightness, contrast, saturation, hue
    ):
        self.consistent_transform = consistent_transform
        self.brightness = (
            brightness
        ...
        self.hue = (
            hue if isinstance(hue, list) or hue is None else ([-hue, hue])
        )
...
class RandomAffine:
    def __init__(
        ...
        """
        self.degrees = (
            degrees if isinstance(degrees, list) else ([-degrees, degrees])
        )
        self.scale = scale
        self.shear = (
            shear
            if isinstance(shear, list)
            else ([-shear, shear] if shear else None)
        )
    ...
    def __call__(self, datapoint: Datapoint, **kwargs):
        ...
        for idx, img in enumerate(datapoint.images):
            if not self.consistent_transform:
                ...
                log_ratio = torch.log(torch.tensor(self.aspect_ratio_range))
                scale_factor = (
                    torch.empty(1).uniform_(*self.scale_range).item()
                )
                aspect_ratio = torch.exp(
                    torch.empty(1).uniform_(log_ratio[0], log_ratio[1])
                ).item()
            ...
            datapoint = crop(
                datapoint, idx, (crop_x, crop_y, crop_width, crop_height)
            )
