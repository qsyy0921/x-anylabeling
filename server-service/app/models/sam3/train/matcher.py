...
def _do_matching(
    cost, repeats=1, return_tgt_indices=False, do_filtering=False
):
    if repeats > 1:
    ...
    if do_filtering:
        ...
        valid_thresh = 1e8
        valid_ijs = [
            (ii, jj) for ii, jj in zip(i, j) if cost[ii, jj] < valid_thresh
        ]
    ...
    def forward(self, outputs, batched_targets):
        ...
        if "positive_map" in batched_targets:
            ...
            if self.focal_loss:
                ...
                cost_class = -(
                    out_prob.unsqueeze(1) * positive_map.unsqueeze(0)
                ).sum(-1)
        else:
        ...
        costs = [
            c[i]
            for i, c in enumerate(np.split(C, sizes.cpu().numpy(), axis=-1))
        ]
    ...
    def forward(self, outputs, batched_targets, repeats=0, repeat_batch=1):
        ...
        if repeat_batch != 1:
            raise NotImplementedError(
                "please use BinaryHungarianMatcherV2 instead"
            )
...
class BinaryFocalHungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, batched_targets, repeats=1, repeat_batch=1):
        ...
        if self.stable:
            ...
            out_prob = (
                out_prob.unsqueeze(-1).expand_as(cost_bbox) * rescaled_giou
            )
            cost_class = -self.alpha * (
                1 - out_prob
            ) ** self.gamma * torch.log(out_prob) + (
                1 - self.alpha
            ) * out_prob**self.gamma * torch.log(
                1 - out_prob
            )
        else:
...
class BinaryHungarianMatcherV2(nn.Module):
    ...
    def forward(
        ...
        if repeat_batch > 1:
            ...
            if target_is_valid_padded is not None:
                target_is_valid_padded = target_is_valid_padded.repeat(
                    repeat_batch, 1
                )
        ...
        if not self.focal:
            ...
            if self.stable:
                ...
                # directly computing log sigmoid (more numerically stable)
                log_out_prob = torch.nn.functional.logsigmoid(out_score)
                log_one_minus_out_prob = torch.nn.functional.logsigmoid(
                    -out_score
                )
                cost_class = (
                    -self.alpha * (1 - out_prob) ** self.gamma * log_out_prob
        ...
        do_filtering = (
            out_is_valid is not None or target_is_valid_padded is not None
        )
        if out_is_valid is not None:
        ...
        return_tgt_indices = (
            do_filtering
            or torch.any(num_queries < num_boxes * max(repeats, 1)).item()
        )
        if len(costs) == 0:
            ...
            for i in range(1, len(tgt_indices)):
                tgt_indices[i] += sizes[i - 1].item()
            tgt_idx = (
                torch.from_numpy(np.concatenate(tgt_indices)).long().to(device)
            )
        else:
            indices = [
        ...
        if len(indices) > 0:
            src_idx = (
                torch.from_numpy(np.concatenate(indices)).long().to(device)
            )
        else:
