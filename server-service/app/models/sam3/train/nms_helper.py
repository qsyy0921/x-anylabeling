...
def process_track_level_nms(video_groups: Dict, nms_threshold: float) -> Dict:
    ...
    for video_id, tracks in video_groups.items():
        ...
        if track_detections:
            scores = np.array(
                [d["score"] for d in track_detections], dtype=np.float32
            )
            ...
            for idx, track in enumerate(track_detections):
                if idx not in keep:
                    tracks[track["track_idx"]]["bboxes"] = [None] * len(
                        track["bboxes"]
                    )
...
def compute_track_iou_matrix(
    ...
    if HAS_NUMBA:
        iou_matrix = _compute_track_iou_matrix_numba(
            bboxes_stacked, valid_masks, areas
        )
    else:
