...
def mask_intersection(masks1, masks2, block_size=16):
    ...
    for i in range(0, masks1.shape[0], block_size):
        for j in range(0, masks2.shape[0], block_size):
            intersection = (
                (
                    masks1[i : i + block_size, None]
                    * masks2[None, j : j + block_size]
                )
                .flatten(-2)
                .sum(-1)
            )
...
def dilation(mask, kernel_size):
    ...
    if mask.is_cuda:
        ...
        result = torch.nn.functional.conv2d(
            result, k.transpose(-1, -2), padding="same"
        )
        return result.view_as(mask) > 0

    all_masks = (
        mask.view(-1, mask.size(-2), mask.size(-1)).numpy().astype(np.uint8)
    )
...
def compute_F_measure(
    gt_boundary_rle,
    gt_dilated_boundary_rle,
    dt_boundary_rle,
    dt_dilated_boundary_rle,
):
    ...
    """
    gt_match = maskUtils.merge(
        [gt_boundary_rle, dt_dilated_boundary_rle], True
    )
    dt_match = maskUtils.merge(
        [dt_boundary_rle, gt_dilated_boundary_rle], True
    )
