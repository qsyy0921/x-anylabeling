...
class Visualizer:
    ...
    def __init__(
        ...
        self._default_font_size = (
            max(
                np.sqrt(self.output.height * self.output.width) // 60,
                15 // scale,
            )
    ...
    def draw_dataset_dict(self, dic):
        ...
        if annos:
            ...
            boxes = [
                (
                    BoxMode.convert(
                        x["bbox"], x["bbox_mode"], BoxMode.XYXY_ABS
                    )
                    if len(x["bbox"]) == 4
                    else x["bbox"]
                )
        ...
        if pan_seg is not None:
            ...
            self.draw_panoptic_seg(
                pan_seg, segments_info, area_threshold=0, alpha=0.7
            )
        return self.output
    ...
    def reposition_label(self, position, cur, binary_mask, move_count):
        ...
        location_diff_threshold = (
            15  # limit for the distance between two labels
        )
    ...
    def draw_box(self, box_coord, alpha=0.5, edge_color="g", line_style="-"):
        ...
        linewidth = (
            max(self._default_font_size / 12, 1)
            * self.boarder_width_multiplier
        )
    ...
    def draw_polygon(self, segment, color, edge_color=None, alpha=0.5):
        ...
        polygon = mpl.patches.Polygon(
            ...
            edgecolor=edge_color,
            linewidth=max(
                self._default_font_size // 15 * self.output.scale, 1
            ),
        )
    ...
    def _draw_text_in_mask(self, binary_mask, text, color):
        ...
        Find proper places to draw text given a binary mask.
        """
        _num_cc, cc_labels, stats, centroids = (
