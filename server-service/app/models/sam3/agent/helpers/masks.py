...
class BitMasks:
    ...
    @staticmethod
    def from_roi_masks(
        roi_masks: "ROIMasks", height: int, width: int
    ) -> "BitMasks":
        """
        Args:
            roi_masks:
    ...
    def crop_and_resize(
        self, boxes: torch.Tensor, mask_size: int
    ) -> torch.Tensor:
        """
...
class PolygonMasks:
    ...
    def crop_and_resize(
        ...
        if len(results) == 0:
            return torch.empty(
                0, mask_size, mask_size, dtype=torch.bool, device=device
            )
        ...
        cat_polymasks = type(polymasks_list[0])(
            list(
                itertools.chain.from_iterable(
                    pm.polygons for pm in polymasks_list
                )
            )
        )
