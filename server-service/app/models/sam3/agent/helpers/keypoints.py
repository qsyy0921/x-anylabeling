...
class Keypoints:
    ...
    * v=2: labeled and visible
    ...
    def __init__(
        self, keypoints: Union[torch.Tensor, np.ndarray, List[List[float]]]
    ):
        """
        Arguments:
        ...
        assert (
            keypoints.dim() == 3 and keypoints.shape[2] == 3
        ), keypoints.shape
    ...
    def to_heatmap(
        self, boxes: torch.Tensor, heatmap_size: int
    ) -> torch.Tensor:
        """
...
def heatmaps_to_keypoints(
    maps: torch.Tensor, rois: torch.Tensor
) -> torch.Tensor:
    """
