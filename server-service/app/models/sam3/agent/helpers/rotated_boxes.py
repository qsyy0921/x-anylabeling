...
class RotatedBoxes(Boxes):
    ...
    def to(self, device: torch.device, non_blocking: bool = False):
        ...
        return RotatedBoxes(
            self.tensor.to(device=device, non_blocking=non_blocking)
        )
    ...
    def normalize_angles(self) -> None:
        ...
        self.tensor = torch.cat(
            (self.tensor[:, :4], angle_tensor[:, None]), dim=1
        )
    ...
    def scale(self, scale_x: float, scale_y: float) -> None:
        ...
        self.tensor[:, 2] *= torch.sqrt(
            (scale_x * c) ** 2 + (scale_y * s) ** 2
        )
        ...
        self.tensor[:, 3] *= torch.sqrt(
            (scale_x * s) ** 2 + (scale_y * c) ** 2
        )
        ...
        self.tensor[:, 4] = (
            torch.atan2(scale_x * s, scale_y * c) * 180 / math.pi
        )
