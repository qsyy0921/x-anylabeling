...
def _decode_single_mask(mask_repr, h: int, w: int) -> np.ndarray:
    ...
    if not isinstance(mask_repr, (str, bytes)):
        raise ValueError(
            "Unsupported mask representation type for RLE decode."
        )

    rle = {
        "counts": (
            mask_repr
            if isinstance(mask_repr, (str, bytes))
            else str(mask_repr)
        ),
        "size": [h, w],
    }
...
def remove_overlapping_masks(sample: Dict, iom_thresh: float = 0.3) -> Dict:
    ...
    if "pred_masks" not in sample or not isinstance(
        sample["pred_masks"], list
    ):
    ...
    assert N == len(
        pred_scores
    ), "pred_masks and pred_scores must have same length"
    if pred_boxes is not None:
        assert N == len(
            pred_boxes
        ), "pred_masks and pred_boxes must have same length"
    ...
    out["removed_indices"] = [
        i for i in range(N) if i not in set(kept_idx_sorted)
    ]
