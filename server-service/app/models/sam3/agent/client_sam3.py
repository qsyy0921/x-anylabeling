...
def call_sam_service(
    ...
    os.makedirs(
        os.path.join(output_folder_path, image_path.replace("/", "-")),
        exist_ok=True,
    )
    ...
    try:
        ...
        serialized_response = sam3_inference(
            sam3_processor, image_path, text_prompt
        )
        ...
        if (
            "pred_scores" in serialized_response
            and serialized_response["pred_scores"]
        ):
