...
def save_debug_messages(
    messages_list, debug, debug_folder_path, debug_jsonl_path
):
...
def _prune_messages_for_next_round(
    ...
    part2 = (
        messages_list[part2_start_idx:] if part2_start_idx is not None else []
    )
    ...
    while generated_text is not None:
        ...
        if tool_call["name"] == "segment_phrase":
            ...
            if current_text_prompt in USED_TEXT_PROMPTS:
                ...
                messages.append(
                    {
                        "role": "user",
                        "content": [
                            {"type": "text", "text": duplicate_prompt_message}
                        ],
                    }
                )
            else:
                ...
                if num_masks == 0:
                    print(
                        "❌ No masks generated by SAM3, reporting no mask to Qwen."
                    )
            ...
            for i in range(num_masks):
                ...
                image_w_zoomed_in_mask_i_path = os.path.join(
                    sam_output_dir,
                    rf"{LATEST_SAM3_TEXT_PROMPT}.png".replace("/", "_"),
                ...
                iterative_checking_messages = [
                    {
                        "role": "system",
                        "content": iterative_checking_system_prompt,
                    },
                    {
                        "role": "user",
                        "content": [
                            ...
                            {
                                "type": "image",
                                "image": image_w_zoomed_in_mask_i_path,
                            },
                        ],
                    },
                ]
            ...
            updated_outputs = {
                ...
                "pred_boxes": [
                    current_outputs["pred_boxes"][i] for i in masks_to_keep
                ],
                "pred_scores": [
                    current_outputs["pred_scores"][i] for i in masks_to_keep
                ],
                "pred_masks": [
                    current_outputs["pred_masks"][i] for i in masks_to_keep
                ],
            }
                ...
                messages.append(
                    {
                        ...
                        "content": [
                            ...
                            {
                                "type": "image",
                                "image": image_w_check_masks_path,
                            },
                        ],
                    }
                )
            ...
            json.dump(
                updated_outputs,
                open(PATH_TO_LATEST_OUTPUT_JSON, "w"),
                indent=4,
            )
            ...
            final_outputs = {
                ...
                "pred_scores": [
                    current_outputs["pred_scores"][i - 1]
