...
def union(labels_ptr, a, b, process_mask):
    ...
    while tl_any(~done_mask):
        ...
        final_root = tl.where(
            are_equal & work_mask & ~done_mask, root_a, final_root
        )
        ...
        final_root = tl.where(
            success_b & work_mask & ~done_mask, root_a, final_root
        )
        ...
        current_b = tl.where(
            success_b | ~merge_mask_a_smaller, current_b, old_val_b
        )
        ...
        final_root = tl.where(
            success_a & work_mask & ~done_mask, root_b, final_root
        )
        ...
        current_a = tl.where(
            success_a | ~merge_mask_b_smaller, current_a, old_val_a
        )
...
def _merge_helper(
    ...
    neighbor_labels = tl.load(
        labels_ptr + tl.ravel(base_offset + offsets_neighbor),
        mask=mask_n,
        other=-1,
    )
    ...
    to_merge = (
        mask_n
        & (neighbor_labels != -1)
        & tl.ravel(current_values == neighbor_values)
    )
...
def _local_prop_kernel(
    ...
    offsets_h = (
        pid_hw // tl.cdiv(W, BLOCK_SIZE_W)
    ) * BLOCK_SIZE_H + tl.arange(0, BLOCK_SIZE_H)
    ...
    tl.atomic_min(
        labels_ptr + tl.ravel(base_offset + offsets_2d),
        current_labels,
        mask=mask_1d,
    )
...
def _pointer_jump_kernel(
    ...
    while tl_any(~(done_mask | ~valid_mask)):
        ...
        current_labels = tl.where(
            ~done_mask,
            tl.minimum(current_labels, parent_labels),
            current_labels,
        )
...
def _count_labels_kernel(
    labels_ptr, sizes_ptr, numel, BLOCK_SIZE: tl.constexpr
):
    pid = tl.program_id(0)
    offsets = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offsets < numel
...
def connected_components_triton(input_tensor: torch.Tensor):
    ...
    _pointer_jump_kernel[grid_jump](
        labels, output, numel, BLOCK_SIZE=BLOCK_SIZE
    )
