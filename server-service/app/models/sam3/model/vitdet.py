    ...
    if size != h or size != w:
        ...
        if tiling:
            new_abs_pos = new_abs_pos.tile(
                [1, 1]
                + [x // y + 1 for x, y in zip((h, w), new_abs_pos.shape[2:])]
            )[:, :, :h, :w]
        else:
        ...
        if not retain_cls_token:
            ...
            return torch.cat(
                [
                    cls_pos,
                    new_abs_pos.permute(0, 2, 3, 1).reshape(1, h * w, -1),
                ],
                dim=1,
            )
...
def concat_rel_pos(
    ...
    rel_h = (
        torch.einsum("bhwc,hkc->bhwk", r_q, Rh) * new_scale
    )  # (B, q_h, q_w, k_h)
    rel_w = (
        torch.einsum("bhwc,wkc->bhwk", r_q, Rw) * new_scale
    )  # (B, q_h, q_w, k_w)
    ...
    q = torch.cat([r_q * scale_ratio, rel_h, rel_w], dim=-1).view(
        B, q_h * q_w, -1
    )
...
class ViT(nn.Module):
    ...
    def __init__(
        ...
        window_block_indexes = [
            i for i in range(depth) if i not in global_att_blocks
        ]
        ...
        if self.retain_cls_token:
            ...
            assert (
                sum(self.rel_pos_blocks) == 0
            ), "rel pos not supported with cls token"

            scale = embed_dim**-0.5
            self.class_embedding = nn.Parameter(
                scale * torch.randn(1, 1, embed_dim)
            )
