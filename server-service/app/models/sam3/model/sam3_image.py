...
class Sam3Image(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert num_o2m_static == (
            num_o2o_static if self.transformer.decoder.dac else 0
        )
    ...
    def _get_img_feats(self, backbone_out, img_ids):
        ...
        if "backbone_fpn" in backbone_out:
            ...
            vis_feats = backbone_out["backbone_fpn"][
                -self.num_feature_levels :
            ]
            vis_pos_enc = backbone_out["vision_pos_enc"][
                -self.num_feature_levels :
            ]
            vis_feat_sizes = [
                x.shape[-2:] for x in vis_pos_enc
            ]  # (H, W) shapes
        ...
        id_mapping[unique_ids] = torch.arange(
            len(unique_ids), device=self.device
        )
        backbone_out = {
            **backbone_out,
    ...
    def _encode_prompt(
        ...
        if encode_text:
            prompt = torch.cat(
                [txt_feats, geo_feats, visual_prompt_embed], dim=0
            )
            prompt_mask = torch.cat(
                [txt_masks, geo_masks, visual_prompt_mask], dim=1
            )
        else:
    ...
    def _run_decoder(
        ...
        reference_boxes = reference_boxes.transpose(
            1, 2
    ...
    def _update_scores_and_boxes(
        ...
        out["queries"] = hs[-1][
            :, :num_o2o
        ]  # remove o2m queries if there are any
        # score prediction
        if self.use_dot_prod_scoring:
            dot_prod_scoring_head = self.dot_prod_scoring
            if (
                is_instance_prompt
                and self.instance_dot_prod_scoring is not None
            ):
    ...
    def _run_segmentation_heads(
        ...
        if self.segmentation_head is not None:
            ...
            seg_head_outputs = activation_ckpt_wrapper(self.segmentation_head)(
                ...
                and self.use_act_checkpoint_seg_head,
                prompt=prompt,
                prompt_mask=prompt_mask,
            )
            aux_masks = (
                False  # self.aux_loss and self.segmentation_head.aux_masks
            )
    ...
    def forward_grounding(
        ...
        with torch.profiler.record_function(
            "SAM3Image._run_segmentation_heads"
        ):
    ...
    def _postprocess_out(self, out: Dict, multimask_output: bool = False):
        ...
        if not self.training and multimask_output and num_mask_boxes > 1:
            ...
            batch_idx = torch.arange(
                len(best_mask_idx), device=best_mask_idx.device
            )
    ...
    def _get_dummy_prompt(self, num_prompts=1):
        ...
        geometric_prompt = Prompt(
            ...
            box_mask=torch.zeros(
                num_prompts, 0, device=device, dtype=torch.bool
            ),
        )
    ...
    def forward(self, input: BatchedDatapoint):
        ...
        num_interactive_steps = (
            0 if self.training else self.num_interactive_steps_val
        )
        geometric_prompt = Prompt(
    ...
    def predict_inst(
        ...
        **kwargs,
        ...
        vision_feats[-1] = (
            vision_feats[-1]
            + self.inst_interactive_predictor.model.no_mem_embed
        )
        feats = [
    ...
    def predict_inst_batch(
        ...
        **kwargs,
        ...
        self.inst_interactive_predictor._orig_hw = [
            (orig_h, orig_w)
            for orig_h, orig_w in zip(orig_heights, orig_widths)
        ]
...
class Sam3ImageOnVideoMultiGPU(Sam3Image):
    ...
    def forward_video_grounding_multigpu(
        ...
        **kwargs,
        ...
        if frame_idx not in multigpu_buffer:
            with torch.profiler.record_function(
                "build_multigpu_buffer_next_chunk1"
            ):
        ...
        if not track_in_reverse and frame_idx_curr_b - self.world_size >= 0:
            ...
            frame_idx_prev_e = min(
                frame_idx_prev_b + self.world_size, num_frames
            )
        else:
        ...
        if not track_in_reverse and frame_idx_curr_e < num_frames:
            ...
            frame_idx_next_e = min(
                frame_idx_next_b + self.world_size, num_frames
            )
        ...
        if (
            ...
            with torch.profiler.record_function(
                "build_multigpu_buffer_next_chunk2"
            ):
                self._build_multigpu_buffer_next_chunk(
                    backbone_out=backbone_out,
                    find_inputs=find_inputs,
    ...
    def _build_multigpu_buffer_next_chunk(
        ...
        frame_idx_local_gpu = min(
            frame_idx_begin + self.rank, frame_idx_end - 1
        )
        ...
        if run_nms:
            with torch.profiler.record_function("nms_masks"):
                ...
                assert (
                    nms_prob_thresh is not None and nms_iou_thresh is not None
                )
                ...
                for prompt_idx in range(pred_probs.size(0)):
                    ...
                    out_local["pred_logits"][prompt_idx, :, 0] -= (
                        1e4 * (~keep).float()
                    )

        if self.gather_backbone_out:
            # gather the SAM 2 backbone features across GPUs
            feats = out_local["prev_encoder_out"]["backbone_out"][
                "sam2_backbone_out"
            ]
            assert (
                len(feats["backbone_fpn"]) == 3
            ...
            backbone_fpn_bf16 = [
                x.to(torch.bfloat16) for x in feats["backbone_fpn"]
            ]
        ...
        out_gathered = {
            k: self._gather_tensor(v) for k, v in out_local.items()
        }
        for rank in range(self.world_size):
            ...
            if self.gather_backbone_out:
                ...
                frame_buffer["tracker_backbone_fpn_0"] = (
                    fpn0[rank],
                    fpn_handle0,
                )
                frame_buffer["tracker_backbone_fpn_1"] = (
                    fpn1[rank],
                    fpn_handle1,
                )
                frame_buffer["tracker_backbone_fpn_2"] = (
                    fpn2[rank],
                    fpn_handle2,
                )
                frame_buffer["tracker_backbone_pos_enc"] = (
                    vision_pos_enc,
                    None,
                )
    ...
    def _gather_tensor(self, x):
        ...
        handle = torch.distributed.all_gather(
            output_list, x, async_op=async_op
        )
