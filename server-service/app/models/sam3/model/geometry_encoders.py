...
def concat_padded_sequences(
    seq1, mask1, seq2, mask2, return_index: bool = False
):
    """
    ...
    concatenated_mask = (
        torch.arange(max_length, device=seq2.device)[None].repeat(
            batch_size, 1
        )
        >= final_lengths[:, None]
    )
    ...
    index = torch.arange(seq2_length, device=seq2.device)[:, None].repeat(
        1, batch_size
    )
...
class Prompt:
    ...
    def __init__(
        ...
        # Dimension checks
        assert box_embeddings is not None and list(
            box_embeddings.shape[:2]
        ) == [
            box_seq_len,
            bs,
        ...
        assert point_embeddings is not None and list(
    ...
    def clone(self):
        return Prompt(
            box_embeddings=(
                None
                if self.box_embeddings is None
                else self.box_embeddings.clone()
            ),
            box_mask=None if self.box_mask is None else self.box_mask.clone(),
            point_embeddings=(
                None
                if self.point_embeddings is None
                else self.point_embeddings.clone()
            ),
            point_mask=(
                None if self.point_mask is None else self.point_mask.clone()
            ),
            box_labels=(
                None if self.box_labels is None else self.box_labels.clone()
            ),
            point_labels=(
                None
                if self.point_labels is None
                else self.point_labels.clone()
            ),
        )
...
class SequenceGeometryEncoder(nn.Module):
    ...
    def __init__(
        ...
        if (
            self.points_pool_project is not None
            or self.boxes_pool_project is not None
        ):
    ...
    def forward(
        self, geo_prompt: Prompt, img_feats, img_sizes, img_pos_embeds=None
    ):
        ...
        if self.cls_embed is not None:
            cls = self.cls_embed.weight.view(1, 1, self.d_model).repeat(
                1, bs, 1
            )
