...
def load_resource_as_video_frames(
    ...
    if isinstance(resource_path, list):
        ...
        assert all(
            isinstance(img_pil, Image.Image) for img_pil in resource_path
        )
        ...
        for img_pil in resource_path:
            img_np = np.array(
                img_pil.convert("RGB").resize((image_size, image_size))
            )
            assert (
                img_np.dtype == np.uint8
            ), "np.uint8 is expected for JPEG images"
            img_np = img_np / 255.0
...
def load_image_as_single_frame_video(
    ...
    images, image_height, image_width = _load_img_as_tensor(
...
def load_video_frames(
    ...
    if video_path.startswith("<load-dummy-video"):
        ...
        return load_dummy_video(
            image_size, offload_video_to_cpu, num_frames=num_frames
        )
...
def load_video_frames_from_image_folder(
    ...
    img_paths = [
        os.path.join(image_folder, frame_name) for frame_name in frame_names
    ]
    img_mean = torch.tensor(img_mean, dtype=torch.float16)[:, None, None]
    img_std = torch.tensor(img_std, dtype=torch.float16)[:, None, None]
...
def load_video_frames_from_video_file_using_cv2(
    ...
    video_tensor = torch.from_numpy(frames_np).permute(
        0, 3, 1, 2
    )  # (T, C, H, W)
...
class TorchCodecDecoder:
    ...
    def __init__(
        self, source, dimension_order="NCHW", device="cpu", num_threads=1
    ):
        from torchcodec import _core as core

        self._source = (
            source  # hold a reference to the source to prevent it from GC
        )
...
class AsyncVideoFileLoaderWithTorchCodec:
    ...
    def __init__(
        ...
        if offload_video_to_cpu:
            ...
            out_device = (
                torch.device("cuda") if gpu_device is None else gpu_device
            )
        ...
        if not isinstance(img_mean, torch.Tensor):
            img_mean = torch.tensor(img_mean, dtype=torch.float16)[
                :, None, None
            ]
        self.img_mean = img_mean
        ...
        if gpu_acceleration:
            ...
            decoder_option = {
                "num_threads": 1
            }  # use a single thread to save memory
    ...
    def _transform_frame(self, frame):
        frame = (
            frame.clone()
        )  # make a copy to avoid modifying the original frame bytes
        ...
        if self.offload_video_to_cpu:
            ...
            frame_resized = frame_resized.to(
                device=self.out_device, non_blocking=True
            )
        return frame_resized
    ...
    def __getitem__(self, index):
        ...
        raise RuntimeError(
            f"Failed to load frame {index} after {max_tries} tries"
        )
