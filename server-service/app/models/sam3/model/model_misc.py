...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        if residual and input_dim != output_dim:
            raise ValueError(
                "residual is only supported if input_dim == output_dim"
            )
...
class SAM3Output(list):
    ...
    def __init__(
        ...
        self._mode2iter = {
            SAM3Output.IterMode.ALL_STEPS_PER_STAGE: lambda: iter(
                self_ref().output
            ),
    ...
    def __getitem__(self, index):
        ...
        assert isinstance(
            index, int
        ), f"index should be an integer. Got {type(index)}"
    ...
    def iteration_mode(
        ...
        """
        return SAM3Output._IterationMode(
            model_output=model_output, iter_mode=iter_mode
        )
