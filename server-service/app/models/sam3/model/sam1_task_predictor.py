...
class SAM3InteractiveImagePredictor(nn.Module):
    ...
    def predict_batch(
        ...
        assert (
            self._is_batch
        ), "This function should only be used when in batched mode"
        ...
        for img_idx in range(num_images):
            ...
            point_coords = (
                point_coords_batch[img_idx]
                if point_coords_batch is not None
                else None
            )
            point_labels = (
                point_labels_batch[img_idx]
                if point_labels_batch is not None
                else None
            )
            box = box_batch[img_idx] if box_batch is not None else None
            mask_input = (
                mask_input_batch[img_idx]
                if mask_input_batch is not None
                else None
            )
            ...
            low_res_masks_np = (
                low_res_masks.squeeze(0).float().detach().cpu().numpy()
            )
        ...
        if boxes is not None:
            ...
            if concat_points is not None:
                concat_coords = torch.cat(
                    [box_coords, concat_points[0]], dim=1
                )
                concat_labels = torch.cat(
                    [box_labels, concat_points[1]], dim=1
                )
