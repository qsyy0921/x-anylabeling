    ...
    def __init__(
        ...
        if no_dec:
            ...
            self.mask_predictor = MaskPredictor(
                hidden_dim, mask_dim=hidden_dim
            )
    ...
    def _embed_pixels(
        ...
        if self.use_encoder_inputs:
            if backbone_feats[0].shape[0] > 1:
                ...
                backbone_visual_feats = [
                    bb_feat.clone() for bb_feat in backbone_feats
                ]
            # Extract visual embeddings
            encoder_hidden_states = encoder_hidden_states.permute(1, 2, 0)
            spatial_dim = math.prod(backbone_feats[-1].shape[-2:])
            encoder_visual_embed = encoder_hidden_states[
                ..., :spatial_dim
            ].reshape(-1, *backbone_feats[-1].shape[1:])
...
class UniversalSegmentationHead(SegmentationHead):
    ...
    def __init__(
        ...
        self.semantic_seg_head = nn.Conv2d(
            self.pixel_decoder.out_dim, 1, kernel_size=1
        )
