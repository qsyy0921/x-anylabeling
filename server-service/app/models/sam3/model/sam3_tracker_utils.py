...
def sample_one_point_from_error_center_slow(
    gt_masks, pred_masks, padding=True
):
    """
    Sample 1 random point (along with its label) from the center of each error region,
    that is, the point with the largest distance to the boundary of each error region.
    ...
    if (
        ...
        if keep_first_cond_frame:
            ...
            if idx_first is None:
                ...
                idx_first = max(
                    (t for t in cond_frame_outputs if t > frame_idx),
                    default=None,
                )
...
def get_best_gt_match_from_multimasks(
    pred_multimasks, gt_masks, pred_scores=None
):
    """
...
def fill_holes_in_mask_scores(
    mask, max_area, fill_holes=True, remove_sprinkles=True
):
    """
    ...
    if remove_sprinkles:
        ...
        mask_fg = mask > 0
        fg_area_thresh = torch.sum(
            mask_fg, dim=(2, 3), keepdim=True, dtype=torch.int32
        )
