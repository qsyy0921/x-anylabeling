...
class Sam3TrackerBase(torch.nn.Module):
    ...
    def _forward_sam_heads(
        ...
        if self.training and self.teacher_force_obj_scores_for_mem:
            ...
            is_obj_appearing = torch.any(
                gt_masks.float().flatten(1) > 0, dim=1
            )
    ...
    def _use_mask_as_output(
        self, backbone_features, high_res_features, mask_inputs
    ):
        """
    ...
    def _prepare_backbone_features_per_frame(self, img_batch, img_ids):
        ...
        if img_ids.numel() > 1:
            unique_img_ids, inv_ids = torch.unique(
                img_ids, return_inverse=True
            )
        else:
    ...
    def frame_filter(
        ...
        for i in range(start, end, step):
            if (
                ...
                or "eff_iou_score"
                not in output_dict["non_cond_frame_outputs"][i]
            ):
                continue

            score_per_frame = output_dict["non_cond_frame_outputs"][i][
                "eff_iou_score"
            ]
    ...
    def _prepare_memory_conditioned_features(
        ...
        if not is_init_cond_frame and use_prev_mem_frame:
            ...
            for t_pos, prev, is_selected_cond_frame in t_pos_and_prevs:
                ...
                if (
                    ...
                    and getattr(self, "cond_frame_spatial_embedding", None)
                    is not None
                ):
                    # add a spatial embedding for the conditioning frame
                    maskmem_enc = (
                        maskmem_enc + self.cond_frame_spatial_embedding
                    )
                ...
                maskmem_enc = (
                    maskmem_enc
                    + self.maskmem_tpos_enc[self.num_maskmem - t - 1]
                )
            ...
            if len(pos_and_ptrs) > 0:
                ...
                if (
                    getattr(self, "cond_frame_obj_ptr_embedding", None)
                    is not None
                ):
                    obj_ptrs = (
                        obj_ptrs
                        ...
                        * torch.tensor(
                            is_selected_cond_frame_list, device=device
                        )[..., None, None].float()
                    )
        ...
        pix_feat_with_mem = (
            encoder_out["memory"].permute(1, 2, 0).view(B, C, H, W)
        )
        return pix_feat_with_mem
    ...
    def forward_tracking(self, backbone_out, input, return_dict=False):
        ...
        frames_to_add_correction_pt = backbone_out[
            "frames_to_add_correction_pt"
        ]
        # first process all the initial conditioning frames to encode them as memory,
        # and then conditioning on them to track the remaining frames
        processing_order = (
            init_cond_frames + backbone_out["frames_not_in_init_cond"]
        )
        output_dict = {
        ...
        for stage_id in processing_order:
            ...
            current_out = self.track_step(
                ...
                mask_inputs=backbone_out["mask_inputs_per_frame"].get(
                    stage_id, None
                ),
                gt_masks=backbone_out["gt_masks_per_frame"].get(
                    stage_id, None
                ),
    ...
    def track_step(
        ...
        if self.offload_output_to_cpu_for_eval and not self.training:
            ...
            trimmed_out = {
                ...
                "pred_masks_high_res": current_out[
                    "pred_masks_high_res"
                ].cpu(),
        ...
        if self.trim_past_non_cond_mem_for_eval and not self.training:
            ...
            if (
                self.use_memory_selection
                and not self.offload_output_to_cpu_for_eval
            ):  ## design for memory selection, trim too old frames to save memory
                far_old_frame_idx = (
                    frame_idx - 20 * self.max_obj_ptrs_in_encoder
                )
                past_out = output_dict["non_cond_frame_outputs"].get(
    ...
    def _use_multimask(self, is_init_cond_frame, point_inputs):
        ...
        num_pts = (
            0 if point_inputs is None else point_inputs["point_labels"].size(1)
        )
        multimask_output = (
            self.multimask_output_in_sam
            and (is_init_cond_frame or self.multimask_output_for_tracking)
            and (
                self.multimask_min_pt_num
                <= num_pts
