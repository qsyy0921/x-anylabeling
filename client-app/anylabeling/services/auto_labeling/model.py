...
class Model(QObject):
    ...
    def get_model_abs_path(self, model_config, model_path_field_name):
        ...
        if os.getenv("XANYLABELING_REMOTE_ONLY", "").lower() in {
            "1",
            "true",
            "yes",
        }:
            raise RuntimeError(
                QCoreApplication.translate(
                    "Model",
                    "This client is configured for server-only inference. "
                    "Select Remote-Server instead of downloading a local model.",
                )
            )
