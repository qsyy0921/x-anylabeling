...
class LabelingWidget(LabelDialog):
    ...
    def __init__(  # noqa: C901
        ...
        open_server_dir = action(
            self.tr("Open Server Dataset"),
            self.open_server_dataset,
            None,
            "open",
            self.tr("Download and open a dataset stored on the server"),
        )
        upload_server_model = action(
            self.tr("Upload Model to Server"),
            self.upload_model_to_server,
            None,
            "label",
    ...
    def open_server_dataset(self):
        ...
        try:
            ...
            if not choices:
                QMessageBox.information(
                    self,
                    self.tr("Server Dataset"),
                    self.tr("No server image directories are available."),
                )
                return

            server_path, accepted = QtWidgets.QInputDialog.getItem(
                self,
                self.tr("Open Server Dataset"),
            ...
            QMessageBox.critical(
                self,
                self.tr("Server Dataset Error"),
                str(exc),
            )
        finally:
            QtWidgets.QApplication.restoreOverrideCursor()

    def upload_model_to_server(self):
        archive_path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self,
            self.tr("Upload Model to Server"),
            ".",
            self.tr("ZIP model package (*.zip)"),
        )
        if not archive_path:
            return
        try:
            ...
            QtWidgets.QApplication.processEvents()
            result = RemoteStorageClient().upload_model(Path(archive_path))
            QMessageBox.information(
                self,
                self.tr("Model Uploaded"),
                self.tr(
                    "The model package was uploaded to server staging.\n"
                    "Package ID: %s\n"
                    "Administrative review is required before activation."
                )
                % result["package_id"],
            )
        except Exception as exc:
            logger.exception("Failed to upload model package")
            QMessageBox.critical(
                self,
                self.tr("Model Upload Error"),
                str(exc),
            )
        finally:
            QtWidgets.QApplication.restoreOverrideCursor()
